...
#
module PursePayment
  class ActivationByUser < Deposit
    has_one :purse_payment_activation_from_user,
    ...
    def action_after_complete
      if user.payment? || user.investor?
        # auto payment activation by parent
        params = {
          source_purse: target.user.parent.try(:purse),
          purse: purse,
          amount: - amount,
          source_payment: self
        }
        payment = target.purse_payment_activation_from_users.create(params)
